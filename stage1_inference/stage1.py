...
def _serialize_window_detections(
    window: list[TrackCrop],
    detections_by_frame: dict[int, Detection],
) -> str:
    track_window = []
    for crop in window:
        det = detections_by_frame[crop.frame_id]
        track_window.append(
            {
    ...
    return json.dumps(track_window, separators=(",", ":"))
...
def run_video_inference(
    *,
    ...
    selected_detections_by_frame = {
        det.frame_id: det for det in detections if det.pedestrian_id == selected_pedestrian_id
    }
            ...
            row.update(
                {
                    ...
                    "track_window_json": _serialize_window_detections(window, selected_detections_by_frame),
                }
            )
            rows.append(row)
